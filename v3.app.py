# -*- coding: utf-8 -*-
"""
Streamlit おしゃれLPジェネレーター - AI生成 + Editor + AI差分編集（OpenAI SDK v1以降）

セットアップ:
- pip install -U openai streamlit python-dotenv
- .env に OPENAI_API_KEY を保存
- streamlit run v3.app.py
"""
...
# ページ設定 / UI
# ────────────────────────────────────────────────────────────────────────────
st.set_page_config(page_title="おしゃれLPジェネレーター（AI + Editor）", page_icon="✨", layout="wide")
st.title("✨ おしゃれLPジェネレーター（AI生成 + Editor）")
st.caption("毎回“別物UI”をAIが生成 → その場で色/テキスト/画像を編集 → さらにAI差分編集 → ZIPでNetlifyへ")
...
with st.sidebar:
    ...
    testi_raw   = st.text_area("お客様の声（名前|肩書き|コメント を改行）", height=80,
...
def ai_generate(theme, site_title, tagline, meta_desc, email, about, feats, works, testi, temperature) -> Tuple[str,str]:
...
if gen_clicked:
    try:
        ...
        st.error("レート上限/クォータに達しました（Billingをご確認ください）。"); st.exception(e); st.stop()
...
<style>{st.session_state.gen_css}</style></head><body>{body_only}</body></html>"""
    ...
    with colB:
        ...
        with st.expander("🤖 AIに指示して再編集（差分適用）", expanded=False):
            st.write("例）『ヒーローを左右2カラムに』『可愛い方向に』『角丸を20px』『余白を広め』『ネオン感を強く』など")
...
"""
            ...
            if st.button("🪄 指示どおりAIで再編集する"):
                try:
                    curr_body = st.session_state.gen_html.split("<body>",1)[1].rsplit("</body>",1)[0]
                    new_css, new_body = ai_edit(st.session_state.gen_css, curr_body, ai_edit_prompt.strip() or "全体を洗練。余白と階層のコントラストを調整。")
                    if new_css: st.session_state.gen_css = new_css
                    if new_body:
        ...
        st.download_button("📦 一括ZIP（Netlify用）", to_zip(st.session_state.gen_html, st.session_state.gen_css),
